...
pub fn djikstra_search<T, G>(start: &T, goal: &T, graph: &G) -> Option<Vec<T>>
where
    T: Clone + Hash + Eq + PartialEq + std::fmt::Debug,
    G: Searchable<T>,
{
    a_star_search(start, goal, graph, |_a, _b| 0)
}

pub fn a_star_search<T, G, H>(start: &T, goal: &T, graph: &G, heuristic: H) -> Option<Vec<T>>
where
    T: Clone + Hash + Eq + PartialEq + std::fmt::Debug,
    G: Searchable<T>,
    H: Fn(&T, &T) -> usize,
